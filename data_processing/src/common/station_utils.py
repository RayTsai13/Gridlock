...
def activity_proxy_from_density_and_connectivity(
    df: pd.DataFrame,
    density_column: str = "residential_density_ratio",
    connectivity_column: str = "connectivity_raw",
    density_weight: float = 0.7,
    connectivity_weight: float = 0.3,
) -> pd.Series:
    density = pd.to_numeric(
        df.get(density_column, pd.Series(0, index=df.index)), errors="coerce"
    ).fillna(0)
    connectivity = pd.to_numeric(
        df.get(connectivity_column, pd.Series(0, index=df.index)), errors="coerce"
    )
    connectivity_rank = connectivity.rank(pct=True).fillna(0)
    return density * density_weight + connectivity_rank * connectivity_weight
