...
def build_station_vectors(trips: pd.DataFrame, density_vectors: pd.DataFrame) -> pd.DataFrame:
    stations = pd.concat(
        [
            trips[["from_station_id", "from_station_clean"]].rename(
                columns={"from_station_id": "station_id", "from_station_clean": "station_clean"}
            ),
            trips[["to_station_id", "to_station_clean"]].rename(
                columns={"to_station_id": "station_id", "to_station_clean": "station_clean"}
            ),
        ],
        ignore_index=True,
    )
    stations = stations.groupby("station_id", as_index=False).agg(station_clean=("station_clean", "first"))

    origin = (
        trips.groupby("from_station_id")
        .agg(
            origin_trips=("TripID", "count"),
            origin_passengers=("Passengers", "sum"),
            outbound_connections=("to_station_id", "nunique"),
        )
        .reset_index()
        .rename(columns={"from_station_id": "station_id"})
    )
    destination = (
        trips.groupby("to_station_id")
        .agg(
            destination_trips=("TripID", "count"),
            destination_passengers=("Passengers", "sum"),
            inbound_connections=("from_station_id", "nunique"),
        )
        .reset_index()
        .rename(columns={"to_station_id": "station_id"})
    )

    vectors = stations.merge(origin, on="station_id", how="left").merge(destination, on="station_id", how="left")
    numeric_columns = [
        "origin_trips",
        "origin_passengers",
    ...
    vectors["connectivity_raw"] = vectors["outbound_connections"] + vectors["inbound_connections"]
